<h1>🚀 Разработка Системы Управления Банковскими Картами</h1>

<h2>📁 Структура</h2>
  <p>
   Приложение для управления банковскими картами.
  </p>
<h2>📝 Запуск приложения через docker-compose.yml</h2>
  <code>docker-compose --profile dev up -d</code>
  Запускает контейнер приложения, которое доступно по порту 8080, и контейнер для postgres.
<h2>📝 Описание</h2>
  <p>Backend-приложение на Java 21 (Spring Boot) для управления банковскими картами:</p>
  <ul>
    <li>Создание и управление картами</li>
    <li>Просмотр карт</li>
    <li>Переводы между своими картами</li>
  </ul>

<h2>💳 Атрибуты карты</h2>
  <ul>
    <li>Номер карты (зашифрован, отображается маской: <code>**** **** **** 1234</code>)</li>
    <li>Владелец</li>
    <li>Срок действия</li>
    <li>Статус: Активна, Заблокирована, Истек срок</li>
    <li>Баланс</li>
  </ul>

<h3>✅ Аутентификация и авторизация</h3>
  <ul>
    <li>Spring Security + JWT</li>
    <li>Роли: <code>ADMIN</code> и <code>USER</code></li>
  </ul>

<h3>✅ Возможности</h3>
<strong>Администратор:</strong>
  <ul>
    <li>Создаёт, блокирует, активирует, удаляет карты</li>
    <li>Управляет пользователями</li>
    <li>Видит все карты</li>
  </ul>

<strong>Пользователь:</strong>
  <ul>
    <li>Просматривает свои карты (поиск + пагинация)</li>
    <li>Делает переводы между своими картами</li>
    <li>Смотрит данные своих карт</li>
  </ul>

<h3>✅ API</h3>
  <ul>
    <li>CRUD для карт</li>
    <li>Переводы между своими картами</li>
    <li>Фильтрация и постраничная выдача</li>
    <li>Валидация и сообщения об ошибках</li>
  </ul>

<h3>✅ Безопасность</h3>
  <ul>
    <li>Шифрование данных</li>
    <li>Ролевой доступ</li>
    <li>Маскирование номеров карт</li>
  </ul>

<h3>✅ Хранение карт в PostgreSQL</h3>
  <ul>
    <li>В PostgreSQL создаются таблицы <code>bank-cards</code> для хранения кард и <code>user_details</code> для хранения информации о пользователях (имена, логины, пароли, роли)</li>
    <li>Миграции через Liquibase</li>
    <li>При запуске приложения, кроме таблиц, также в таблицу <code>user_details</code> добавляется информация о пользователе с ролью <code>ADMIN</code></li>
  </ul>

<h3>✅ Развёртывание и тестирование</h3>
  <ul>
    <li>Docker Compose для dev-среды</li>
    <li>Liquibase миграции</li>
    <li>Юнит-тесты ключевой service </li>
  </ul>

<h2>💡 Технологии</h2>
  <p>
    Java 21, Spring Boot 4.0, Spring Security, Spring Data JPA, PostgreSQL, Liquibase, Docker, JWT, Swagger (OpenAPI)
  </p>
